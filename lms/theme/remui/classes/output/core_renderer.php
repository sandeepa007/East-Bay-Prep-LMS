...
class core_renderer extends remui_renderer
{
    ...
    public function block(block_contents $bc, $region)
    {
        ...
        $context->subname = $bc->attributes['data-block'];
        
        // See if the current page is mydashboard
        // and calculate block size only if the current page is dashboard
        if ('mydashboard' == $PAGE->pagelayout) {
            // Add subname to block to distinguish it from other remuiblck blocks
            if ('remuiblck' == $context->type) {
                $context->subname = $bc->attributes['data-subname'];
            }
 
            // Check Resize is locked for current block
            if (\theme_remui\utility::check_resize_locked($bc) == 'true') {
                $context->lock = true;
            }
            // retrieve the block size of current block_content i.e $bc
            $size = \theme_remui\utility::get_block_size_pos($bc);
  
            // See if the returned array is not empty
            if (!empty($size)) {
                // Block width and heigth custom added to block content
                $context->width = $size['wd'];
                $context->height = $size['ht'];

                $context->minwidth = $size['mw'];
                // $context->minheight = $size['mh'];

                // Block x and y pos custom added to block content
                $context->posx = $size['x'];
                $context->posy = $size['y'];
            }
        }
