...
class utility
{
    ...
    public static function get_courses(
        ...
        if ($totalcount) {
            ...
            return count($DB->get_records_sql("SELECT c.id FROM {course} c where id != ? $where ORDER BY c.fullname " . $categorysort, $sql_params));
        } else {
            $courses = $DB->get_records_sql("SELECT ".implode($fields, ',')." FROM {course} c where id != ? $where ORDER BY c.fullname " . $categorysort, $sql_params, $limitfrom, $limitto);
        }
    ...
    public static function get_course_category_selector($category = '', $categorysort = '', $search = '', $mycourses = '', $pageurl)
    {
        ...
        if ($mycourses != '') {
            ...
            $categoryhtml .= "<input type='hidden' name='mycourses' value='0'>";
        }
  ...
  // send customer feedback to RemUI team.
    ...
    public static function get_recent_accessed_courses($limit)
    {
     ...
     /*
        ...
        'mh'   // min height
    */
    public static function get_block_size_pos($bc)
    {
        if ('remuiblck' == $bc->attributes['data-block']) {
            $arr_srch = $bc->attributes['data-subname'];
            $haystack = 'sn';
        } else {
            $arr_srch = $bc->attributes['data-block'];
            $haystack = 'id';
        }

        $blck_pos = get_user_preferences('remuiblck_pos_state');

        if ($blck_pos != '' || !empty($blck_pos)) {
            $arr = json_decode($blck_pos);
            $key = array_search($arr_srch, array_column($arr, $haystack));

            if ($key === false) {
                return array();
            }

            $arr[$key] = self::check_not_empty_field($arr[$key]);
        ...
        return self::return_default_size_pos($arr_srch);
    }


    public static function check_not_empty_field($arr)
    {
        $blck_name = $arr->id;
        if ($arr->id == 'remuiblck') {
            $blck_name = $arr->sn;
        }
        $default = self::return_default_size_pos($blck_name);

        // Check if any value is empty
        if (empty($arr->x) || $arr->x == null) {
            $arr->x = '0';
        }

        if (empty($arr->y) || $arr->y == null) {
             $arr->y = '0';
        }

        if (empty($arr->wd) || $arr->wd == null) {
            $arr->wd = $default['wd'];
        }

        if ($arr->wd < $default['mw']) {
            $arr->wd = $default['mw'];
        }

        if (empty($arr->ht) || $arr->ht == null) {
            $arr->ht = $default['ht'];
        }

        if ($arr->ht < $default['mh']) {
            $arr->ht = $default['mh'];
        }
        
        // add minimum height and minimum width to the block size object and return it
        $arr->mw = $default['mw'];
        $arr->mh = $default['mh'];
        return $arr;
    }
    ...
    'mh'   // min height
    */
    public static function return_default_size_pos($blck_name)
    {
        $blck_name = trim($blck_name);

        $options = [
            'default'    => ['wd'=>  '4', 'ht'=> '4', 'x'=>'0', 'y'=> '0', 'mw'=>  '4', 'mh'=> '4'],
            'addnotes'   => ['wd'=>  '6', 'ht'=> '5', 'x'=>'6', 'y'=> '19', 'mw'=>  '6', 'mh'=> '3'],
            'myoverview' => ['wd'=> '12', 'ht'=> '8', 'x'=>'0', 'y'=> '0', 'mw'=> '12', 'mh'=> '8'],
            // 'userstats'  => ['wd'=> '12', 'ht'=> '7', 'x'=>'0', 'y'=> '9', 'mw'=>  '4', 'mh'=> '8'],
            'enrolledusers' => ['wd'=> '12', 'ht'=> '6', 'x'=>'0', 'y'=> '8', 'mw'=>  '6', 'mh'=> '5'],
            'quizattempts'  => ['wd'=> '12', 'ht'=> '6', 'x'=>'0', 'y'=> '13', 'mw'=>  '6', 'mh'=> '6'],
            'recentforums'  => ['wd'=> '12', 'ht'=> '3', 'x'=>'0', 'y'=> '40', 'mw'=> '6', 'mh'=> '3'],
            'latestmembers' => ['wd'=>  '6', 'ht'=> '5', 'x'=>'0', 'y'=> '19', 'mw'=>  '4', 'mh'=> '4'],
            'recentsection' => ['wd'=> '12', 'ht'=> '3', 'x'=>'0', 'y'=> '38', 'mw'=> '6', 'mh'=> '3'],
            'courseprogress' => ['wd'=> '12', 'ht'=> '8', 'x'=>'0', 'y'=> '24', 'mw'=> '8', 'mh'=> '8'],
            'courseanlytics' => ['wd'=> '12', 'ht'=> '8', 'x'=>'0', 'y'=> '32', 'mw'=> '12', 'mh'=> '8'],
            'recentfeedback' => ['wd'=> '12', 'ht'=> '3', 'x'=>'0', 'y'=> '43', 'mw'=> '6', 'mh'=> '3'],
            ];
        
        $key = $blck_name;
        // if ('remui_blck' == $blck_name) {
        //     $key = $subname;
        // }

        if (!array_key_exists($blck_name, $options)) {
            $key = 'default';
        }
    ...
    // $bc is block_content object
    public static function check_resize_locked($bc)
    {
        $blck_name = $bc->attributes['data-block'];
        if ($blck_name == 'remuiblck') {
            $blck_name = $bc->attributes['data-subname'];
        }
        return self::get_lock_status($blck_name);
    }

    public static function get_lock_status($blck_name)
    {
