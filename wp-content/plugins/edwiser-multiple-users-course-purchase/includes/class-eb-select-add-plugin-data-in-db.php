...
if (!class_exists('EBMUSelectAddPluginDataInDB')) {

    class EBMUSelectAddPluginDataInDB
    {
        /*
         *
                ...
                switch ($licenseData->license) {
                    case 'invalid':
                        ...
                        if (isset($licenseData->activations_left) && $licenseData->activations_left == '0') {
                            ...
                            $activeSite = EBMUSelectGetPluginData::getSiteList($pluginSlug);
...
//                            }
        ...
        public function updateNumberOfSitesUsingLicense($licenseData)
        {
            if (isset($licenseData->sites) && (!empty($licenseData->sites) || $licenseData->sites != '')) {
                update_option('eb_' . $this->pluginSlug . '_license_key_sites', $licenseData->sites);
                update_option('eb_' . $this->pluginSlug . '_license_max_site', $licenseData->license_limit);
            } else {
                update_option('eb_' . $this->pluginSlug . '_license_key_sites', '');
                update_option('eb_' . $this->pluginSlug . '_license_max_site', '');
            }
        }
        ...
        public function licenseMessages($eb_lice_messages)
        {
            ...
            $active_site = EBMUSelectGetPluginData::getSiteList($this->pluginSlug);
            ...
            if (isset($_POST['edd_' . $this->pluginSlug . '_license_key']) &&
                ...
                if (isset($_POST['edd_' . $this->pluginSlug . '_license_key']) &&
                    ...
                    add_settings_error(
                        ...
                        sprintf(__('Please enter license key for %s.', 'ebbp-textdomain'), $this->pluginName),
                        'error'
                    );
                    ...
                    add_settings_error(
                        'eb_' . $this->pluginSlug . '_errors',
                        esc_attr('settings_updated'),
                        sprintf(__('No response from server. Please try again later.', "ebbp-textdomain"), $this->pluginName),
                        'error'
                    );
                    ...
                    add_settings_error(
                        'eb_' . $this->pluginSlug . '_errors',
                        esc_attr('settings_updated'),
                        sprintf(__('License key is not valid. Please check your license key and try again', "ebbp-textdomain"), $this->pluginName),
                        'error'
                    );
        ...
        public function invalidStatusMessages($status, $display)
        {
            if ('invalid' == $status && (!empty($display) || '' != $display)) { //Invalid license key   and site
                add_settings_error(
                    'eb_' . $this->pluginSlug . '_errors',
                    esc_attr('settings_updated'),
                    sprintf(__('License Key for %s is already activated at : ' . $display, 'ebbp-textdomain'), $this->pluginName),
                    'error'
                );
                ...
                add_settings_error(
                    'eb_' . $this->pluginSlug . '_errors',
                    esc_attr('settings_updated'),
                    sprintf(__('Please enter valid license key for %s.', 'ebbp-textdomain'), $this->pluginName),
                    'error'
                );
                ...
                add_settings_error(
                    'eb_' . $this->pluginSlug . '_errors',
                    esc_attr('settings_updated'),
                    sprintf(__('License Key for %s is deactivated', 'ebbp-textdomain'), $this->pluginName),
                    'updated'
                );
            }
        }
        ...
        {
            $renew_link = get_option('eb_' . $this->pluginSlug . '_product_site');
            ...
            if (false !== $status && 'valid' == $status) {
                $buttons = '<input type="submit" class="button-primary" name="edd_' . $this->pluginSlug . '_license_deactivate" value="' . __('Deactivate License', 'ebbp-textdomain') . '" />';
            } elseif ('expired' == $status && (!empty($display) || '' != $display)) {
                $buttons = '<input type = "submit" class = "button-primary" name = "edd_' . $this->pluginSlug . '_license_activate" value = "' . __('Activate License', 'ebbp-textdomain') . '"/>';
                $buttons .= ' <input type = "button" class = "button-primary" name = "edd_' . $this->pluginSlug . '_license_renew" value = "' . __('Renew License', 'ebbp-textdomain') . '" onclick = "window.open( \'' . $renew_link . '\')"/>';
        ...
        {
            if (false !== $status && 'valid' == $status) {
                $license_status = '<span style="color:green;">' . __('Active', 'ebbp-textdomain') . '</span>';
            } elseif (get_option('edd_' . $this->pluginSlug . '_license_status') == 'site_inactive') {
                $license_status = '<span style="color:red;">' . __('Not Active', 'ebbp-textdomain') . '</span>';
            } elseif (get_option('edd_' . $this->pluginSlug . '_license_status') == 'expired' && (!empty($display) || '' != $display)) {
                $license_status = '<span style="color:red;">' . __('Expired', 'ebbp-textdomain') . '</span>';
                ...
                $license_status = '<span style="color:red;">' . __('Invalid Key', 'ebbp-textdomain') . '</span>';
            } else {
                $license_status = '<span style="color:red;">' . __('Not Active ', 'ebbp-textdomain') . '</span>';
            }
